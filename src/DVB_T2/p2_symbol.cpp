                               ...
                               address_freq_deinterleaver* _address)
{
    ...
    delete[] deinterleaved_cell;delete[] show_symbol;delete[] show_data;delete[] est_data;delete[] show_est_data;
    delete[] l1_post_bit;delete[] l1_post_bit_interleaving;
    postSoft.resize(c_p2*8);postSoftInterleaved.resize(c_p2*8);
    l1_post_bit=new unsigned char[c_p2*8]{};l1_post_bit_interleaving=new unsigned char[c_p2*8]{};
    deinterleaved_cell = new complex[c_p2];
    m_lastPlpSignature.clear();
...
{
    if(!preDecoder.decode(deinterleaved_cell,preBits)){emit view_l1_presignalling("L1 PRE: CRC/BCH/LDPC ERROR");return false;}
    ...
    for(int i = 0; i < 168; ++i){
        ...
        case 154:
            l1_pre.num_rf = field;
            if(l1_post.rf == nullptr) l1_post.rf = new l1_postsignalling_rf[7]{};
    ...
    if(l1_pre.l1_post_size<=0 || l1_pre.l1_post_size>c_p2-L1_PRE_CELL || l1_pre.l1_post_mod>3 || l1_pre.l1_post_info_size+32>KBCH_1_2 || l1_pre.num_rf<1 || l1_pre.guard_interval>6 || l1_pre.pilot_pattern>7 || l1_pre.num_data_symbols<1)return false;
...
bool p2_symbol::l1_post_info()
{
    ...
    if(n_post<l1_pre.l1_post_info_size+32+168 || (colums && n_post%colums))return false;
    int c_bit = n_bit;
    ...
    for(int i = 0; i < n_post; ++i){
        ...
        float distance=0;
        switch(n_bit-c_bit){
        case 0:distance=real;break;case 1:distance=imag;break;
        case 2:distance=std::abs(real)-amp4;break;case 3:distance=std::abs(imag)-amp4;break;
        case 4:distance=std::abs(std::abs(real)-amp4)-amp2;break;case 5:distance=std::abs(std::abs(imag)-amp4)-amp2;break;
        }
        const float norms[]={1.f,float(NORM_FACTOR_QPSK),float(NORM_FACTOR_QAM16),float(NORM_FACTOR_QAM64)};
        postSoftInterleaved[mux[idx_mux]+w]=8*distance/norms[l1_pre.l1_post_mod];
        //multiplexer
        ...
        if(c_bit == 0){
            c_bit = n_bit + 1;
            ++p2_l1_post;
            if(i+1<n_post){real = (*p2_l1_post).real();imag = (*p2_l1_post).imag();}
        }
        --c_bit;
    }
    //deinterleaving
    ...
    for (int i = 0; i < n_post; ++i) {
        int j = l + step;
        l1_post_bit[j] = l1_post_bit_interleaving[i];postSoft[j]=postSoftInterleaved[i];
    ...
    if(!postDecoder.decode(l1_post_bit,n_post,l1_pre.l1_post_info_size,l1_pre.l1_post_mod,randomize,postBits,postSoft.data())){
        emit view_l1_postsignalling("L1 POST: CRC/BCH/LDPC ERROR");view_l1_post_update=true;chek_l1_post=false;return false;
    }
    std::copy_n(postBits,l1_pre.l1_post_info_size+32,l1_post_bit);
    //calculate CRC32
    ...
    if(l1_post.plp == nullptr) l1_post.plp = new l1_postsignalling_plp[255]{};
...
//    if(l1_post.dyn.aux_private_dyn == nullptr) l1_post.dyn.aux_private_dyn = new int[l1_post.num_aux];
    ...
    {
        const QStringList modNames={QStringLiteral("QPSK"),QStringLiteral("16-QAM"),
                                    QStringLiteral("64-QAM"),QStringLiteral("256-QAM")};
        const QStringList codeNames={QStringLiteral("1/2"),QStringLiteral("3/5"),
                                     QStringLiteral("2/3"),QStringLiteral("3/4"),
                                     QStringLiteral("4/5"),QStringLiteral("5/6")};
        QString signature;QStringList names;QList<int> indices;
        for(int i=0;i<l1_post.num_plp;++i){
            const auto &plp=l1_post.plp[i];
            const QString mod=(plp.plp_mod>=0 && plp.plp_mod<modNames.size())
                    ? modNames[plp.plp_mod] : QStringLiteral("MOD %1").arg(plp.plp_mod);
            const QString code=(plp.plp_cod>=0 && plp.plp_cod<codeNames.size())
                    ? codeNames[plp.plp_cod] : QStringLiteral("COD %1").arg(plp.plp_cod);
            const QString fec=plp.plp_fec_type==FEC_FRAME_NORMAL
                    ? QStringLiteral("normal") : QStringLiteral("short");
            names.append(QStringLiteral("PLP ID %1 · %2 · %3 · %4")
                         .arg(plp.id).arg(mod,code,fec));
            indices.append(i);
            signature += QStringLiteral("%1:%2:%3:%4;")
                    .arg(plp.id).arg(plp.plp_mod).arg(plp.plp_cod).arg(plp.plp_fec_type);
        }
        if(!names.isEmpty() && signature!=m_lastPlpSignature){
            m_lastPlpSignature=signature;
            emit plps_changed(names,indices);
        }
    }
