...
signals:
    ...
    // Published as soon as a valid L1-post frame lists its PLPs.  This is
    // intentionally independent of BBFRAME/PSI decoding: a valid RF frame
    // can be present even while data FEC is not yet locked.
    void plps_changed(QStringList names, QList<int> indices);

private:
    L1PreDecoder preDecoder;
    L1PostDecoder postDecoder;
    uint8_t postBits[7032]{};
    std::vector<float> postSoft,postSoftInterleaved;
    uint8_t preBits[200]{};
    int fft_size;
    float amp_p2;
    int n_p2;
