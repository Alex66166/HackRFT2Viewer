...
{
    thread->quit(); thread->wait(); delete thread; delete mutex_out;
    if(flag_start){
        delete [] fec_len_bits;
...

{
    ...
    for(int i = 0; i < num_plp; ++i){
        ...
        case 1:
            n_ti[i] = 1;
            p_i[i] = qMax(1,l1_post.plp[i].time_il_length);
            break;
        default:
            break;
        }
...
void time_deinterleaver::execute(int _len_in, complex* _ofdm_cell)
{
    ...
    for (int i = 0; i < num_cells; ++i) {
        if(ti_block_size<=0 || idx_cell>slice_end[plp_id]) { ++ofdm_cell; ++idx_cell; continue; }
        int d=idx_step_ti+idx_row_ti;
        if(l1_post.plp[plp_id].time_il_length==0)
            d=idx_row_ti*num_cols[plp_id][idx_time_il]+idx_step_ti/num_rows_plp;
        int i_address=cell_deint[d],q_address=i_address;
        if(l1_post.plp[plp_id].plp_rotation)
            q_address=i_address%cells_per_fec_block_plp==0 ? i_address+cells_per_fec_block_plp-1 : i_address-1;
